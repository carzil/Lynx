...
from ast import Ly_AST_IdentifierNode, Ly_AST_CodeNode, Ly_AST_IfNode, Ly_AST_WhileNode, Ly_AST_ForNode,\
                   Ly_AST_ElseNode
...
def parse_else(tokens):
    ...
    if token.value == "else":
        token = tokens.next()
        if token.value != "{":
            return Ly_AST_ElseNode(Ly_AST_CodeNode(core.parse_primary(tokens)))
        else:
            body = []
            while token.value != "}":
                token = tokens.next()
...
def parse_if(tokens):
    ...
    if token.value != "{":
        ...
        while token.value != "}":
            ...
            if isinstance(token, Ly_EOFToken):
                Ly_SyntaxError(token.file, token.line, token.pos, token.string, "unexpected EOF")
            body.append(core.parse_primary(tokens))
        res3 = parse_else(tokens)
        if res != None:
...
def parse_while(tokens):
    ...
    token = tokens.next()
    if token.value != "{":
        return Ly_AST_WhileNode(res, Ly_AST_CodeNode([core.parse_primary(tokens)]))
    else:
        body = []
        while token.value != "}":
            token = tokens.next()
...
def parse_for(tokens):
    ...
    tokens.next()
    value = core.parse_primary(tokens)
    tokens.expect(";", next=False)
    tokens.next()
    condition = core.parse_primary(tokens)
    tokens.expect(";", next=False)
    tokens.next()
    step = core.parse_primary(tokens)
    tokens.expect(")", next=False)
    token = tokens.next()
    if token.value != "{":
        return Ly_AST_ForNode(loopv, value, condition, step, Ly_AST_CodeNode([core.parse_primary(tokens)]))
    else:
        body = []
        while token.value != "}":
            token = tokens.next()
